...
_start:
    ...
    ;; if the string does not start with a negative sign, check for positive
    cmp al, "-"
    jne check_positive_sign
    mov r11, 1
    jmp find_string_size
check_positive_sign: ; terminate program if no sign (invalid input)
    cmp al, "+"
    jne last
...
find_string_size:
    ...
    ;; end loop if reached NULL terminator
    cmp al, NULL
    je end_find_string_size
    ;; terminate program if char is not between "0" and "9" (not a digit)
    cmp al, 0x30
    jb last
    cmp al, 0x39
    ja last

    ;; loop if none of the quit conditions were met
    jmp find_string_size
end_find_string_size:
